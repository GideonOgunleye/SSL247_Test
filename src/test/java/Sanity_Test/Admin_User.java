...
@Test (priority = 0, groups = {"Smoke","BS_Smoke","BS_Sanity","Smoke_Firefox","Smoke_Chrome"},dataProviderClass =Test_DataSanity.class, dataProvider="AdminLoginAllSites", enabled = false)
...
try {
	  ...
	  Admin_LoginPageElements.EnterUserName(Username);
	  driver.manage().timeouts().implicitlyWait(50, TimeUnit.SECONDS);
	  test.log(LogStatus.INFO, "Entereed UserName");
	  
	  Admin_LoginPageElements.EnterPassword(Password);
	  driver.manage().timeouts().implicitlyWait(50, TimeUnit.SECONDS);
...
@Test (priority = 6,groups = {"Sanity","BS_DailySanity"},dataProviderClass =Test_DataSanity.class, dataProvider="ReissueCertificate", enabled = false)
	...
	//Search For UK Test User
	 ...
	 try { 
		 ...
		 test.log(LogStatus.INFO, "Clicked Create Account Button");
		  ...
		  String imagePath = test.addScreenCapture(path);
		  test.log(LogStatus.INFO, imagePath);
		  test.log(LogStatus.FAIL, "Form Submition Failed");
		  test.log(LogStatus.INFO, e);
		  driver.navigate().refresh();
		  Assert.fail("Exception " + e);
